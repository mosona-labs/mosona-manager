...
func GetUserById(id int64) (_type.User, error) {
	var user _type.User
	if err := Db.Get(&user, "SELECT id, username, email, is_admin, verified, created_at FROM users WHERE id = $1", id); err != nil {
...
func FindUserByEmail(email string) (_type.UserAuthInfo, error) {

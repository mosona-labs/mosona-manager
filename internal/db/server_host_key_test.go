...
func TestEditServerRejectsStaleSSHHostKeyState(t *testing.T) {
	...
	mock.ExpectExec("UPDATE server_info SET note = $1, provider = $2, cycle = $3, start_time = $4, end_time = $5, amount = $6, auto_renew = $7, bandwidth = $8, traffic = $9, traffic_type = $10, note_public = $11 WHERE sid = $12").
		...
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("UPDATE ssh SET address = $1, port = $2, username = $3, host_key = $4, trust_legacy_host_key = $5 WHERE server_id = $6 AND host_key = $7").
		WithArgs("", 0, "", data.HostKey, false, int64(91), previousHostKey).
...
func TestEditServerPinsLegacySSHHostKeyAndDisablesCompatibility(t *testing.T) {
	database, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherEqual))
	if err != nil {
		t.Fatal(err)
	}
